...
export class JobsResource {
  ...
  async list(params?: JobListParams): Promise<PaginatedResponse<Job>> {
    return this.http.get<PaginatedResponse<Job>>('/api/v2/jobs', params as Record<string, unknown>);
  }

  async *listAll(params?: Omit<JobListParams, 'page'>): AsyncGenerator<Job> {
    yield* paginate<Job>(
      (p) => this.http.get<PaginatedResponse<Job>>('/api/v2/jobs', { ...params, ...p }),
    );
  }
  ...
  async createV2(options: JobCreateOptionsV2): Promise<Job> {
    return this.http.post<Job>('/api/v2/jobs', options as unknown as Record<string, unknown>);
  }

  async reTranscode(options: ReTranscodeOptions): Promise<Job> {
    return this.http.post<Job>('/api/v1/uploads/re-transcode', options as unknown as Record<string, unknown>);
  }
